...
Use the Task tool to spawn a subagent. **Do NOT pass any diff content** - only pass the scope information. The subagent will instruct Codex to fetch its own context.
...
Tell Codex to fetch the changes itself based on the scope:
